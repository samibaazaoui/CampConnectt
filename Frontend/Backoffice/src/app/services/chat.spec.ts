...
describe('ChatService', () => {
  ...
  it('should find all rooms', () => {
    ...
    const req = httpMock.expectOne('http://localhost:8084/api/chat/rooms');
  ...
  it('should identify correct message endpoint after my fix', () => {
    ...
    const req = httpMock.expectOne('http://localhost:8084/api/chat/messages');
  ...
  it('should get participants for a room', () => {
    ...
    const req = httpMock.expectOne('http://localhost:8084/api/chat/rooms/123/participants');
    expect(req.request.method).toBe('GET');
    req.flush([]);
  });
});
