...
export class EventsComponent implements OnInit {
  ...
  // Prediction Modal (existing events)
  showPrediction = signal<boolean>(false);
  predictionLoading = signal<boolean>(false);
  prediction = signal<any>(null);
  predictionEventTitle = signal<string>('');

  // Inline prediction (form)
  formPrediction = signal<any>(null);
  formPredictionLoading = signal<boolean>(false);
  ...
  openModal() {
    ...
    this.eventForm.reset({ nActivities: 0 });
    this.formPrediction.set(null);
  ...
  viewAttendees(ev: any) {
    ...
    this.http.get(`/api/event-participations/event/${ev.id}`).subscribe({
      next: (res: any) => {
  ...
  openPrediction(ev: any) {
    this.predictionEventTitle.set(ev.title);
    this.prediction.set(null);
    this.predictionLoading.set(true);
    this.showPrediction.set(true);
    this.http.get(`/api/recommendations/predict-participation/${ev.id}`).subscribe({
      next: (res: any) => {
        const d = res?.data;
        this.prediction.set(d ? {
          predictedParticipants:   d.predicted_participants,
          predictedAttendanceRate: d.predicted_attendance_rate,
          label:      d.label,
          insight:    d.insight,
      ...
      error: () => {
        this.prediction.set({ error: true });
        this.predictionLoading.set(false);
      }
    });
  }
  ...
  getFormPrediction() {
    ...
    const payload = {
      ...
      day_of_week:    start.getDay() === 0 ? 6 : start.getDay() - 1,
      hour_of_start:  start.getHours(),
      duration_hours: parseFloat(durationHours.toFixed(1)),
      n_activities:   v.nActivities || 0
    };
    this.formPrediction.set(null);
    this.formPredictionLoading.set(true);
    this.http.post('/api/recommendations/predict-participation', payload).subscribe({
      next: (res: any) => {
        const d = res?.data;
        this.formPrediction.set(d ? {
          predictedParticipants:   d.predicted_participants,
          predictedAttendanceRate: d.predicted_attendance_rate,
          label:      d.label,
          insight:    d.insight,
        ...
        this.formPredictionLoading.set(false);
      },
      error: () => { this.formPrediction.set({ error: true }); this.formPredictionLoading.set(false); }
    });
  }
}
