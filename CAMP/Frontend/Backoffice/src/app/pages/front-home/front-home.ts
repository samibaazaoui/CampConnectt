...
@Component({
  ...
  template: `
    ...
    <section class="section-block reco-section" *ngIf="authService.isLoggedIn()">
      <div class="section-header">
        <div class="reco-badge"><i class="fa-solid fa-wand-magic-sparkles"></i> AI-Powered</div>
        <h2 class="section-title">Recommended for You</h2>
        <p class="section-desc">Personalized picks based on your reservations and activity history.</p>
      </div>

      <!-- Loading skeleton -->
      <div *ngIf="recoLoading()" class="card-grid">
        <div *ngFor="let s of [1,2,3,4]" class="card skeleton-card">
          <div class="skeleton-img"></div>
          <div class="card-body">
            <div class="skeleton-line" style="width:60%;height:12px"></div>
            <div class="skeleton-line" style="width:90%;height:18px;margin-top:8px"></div>
            <div class="skeleton-line" style="width:75%;height:12px;margin-top:6px"></div>
          </div>
        </div>
      </div>

      <!-- Recommended Campsites -->
      <div *ngIf="!recoLoading() && recoCampsites().length > 0">
        <h3 class="reco-sub-title"><i class="fa-solid fa-campground"></i> Campsites you might love</h3>
        <div class="card-grid">
          <div *ngFor="let camp of recoCampsites()" class="card reco-card">
            <div class="reco-label"><i class="fa-solid fa-wand-magic-sparkles"></i> Recommended</div>
            <div class="card-img-placeholder" [style.background-image]="camp.image_url ? 'url(' + camp.image_url + ')' : ''">
              <i *ngIf="!camp.image_url" class="fa-solid fa-mountain-sun"></i>
            </div>
            <div class="card-body">
              <div class="card-meta">
                <span class="location"><i class="fa-solid fa-map-pin"></i> {{ camp.location }}</span>
                <span class="price">\${{ camp.nightly_price }}/nt</span>
              </div>
              <h3>{{ camp.name }}</h3>
              <div class="card-actions">
                <button class="btn-action" (click)="openBookingModal(camp)">Book Now</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- Recommended Events -->
      <div *ngIf="!recoLoading() && recoEvents().length > 0" style="margin-top: 2.5rem;">
        <h3 class="reco-sub-title"><i class="fa-solid fa-calendar-star"></i> Events you might enjoy</h3>
        <div class="card-grid">
          <div *ngFor="let ev of recoEvents()" class="card reco-card">
            <div class="reco-label"><i class="fa-solid fa-wand-magic-sparkles"></i> Recommended</div>
            <div class="card-img-placeholder" style="background: linear-gradient(135deg,#1b4332,#40916c);">
              <i class="fa-solid fa-campfire" style="color:rgba(255,255,255,0.5)"></i>
            </div>
            <div class="card-body">
              <div class="card-meta">
                <span class="location"><i class="fa-solid fa-map-pin"></i> {{ ev.location }}</span>
              </div>
              <h3>{{ ev.title }}</h3>
              <p style="font-size:0.8rem;color:#64748b;">{{ ev.start_at | date:'mediumDate' }} → {{ ev.end_at | date:'mediumDate' }}</p>
              <div class="card-actions">
                <button class="btn-action" (click)="goToEvents()">View Event</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div *ngIf="!recoLoading() && recoCampsites().length === 0 && recoEvents().length === 0" class="reco-empty">
        <i class="fa-solid fa-compass fa-2x" style="color:#cbd5e1"></i>
  ...
  styles: [`
    ...
    .reco-section { border-radius: 1.5rem; background: linear-gradient(135deg, #f0fdf4 0%, #ecfdf5 100%); padding: 2.5rem; border: 1px solid #bbf7d0; margin-bottom: 5rem; }
    .reco-badge { display: inline-flex; align-items: center; gap: 0.4rem; background: linear-gradient(135deg, #1b4332, #40916c); color: white; font-size: 0.75rem; font-weight: 700; padding: 0.35rem 0.85rem; border-radius: 20px; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 0.75rem; }
    .reco-sub-title { font-size: 1.15rem; font-weight: 700; color: #1b4332; margin-bottom: 1.25rem; display: flex; align-items: center; gap: 0.5rem; }
    .reco-card { position: relative; border: 2px solid #86efac; }
    .reco-card:hover { border-color: #40916c; }
    .reco-label { position: absolute; top: 0.75rem; left: 0.75rem; background: linear-gradient(135deg, #1b4332, #40916c); color: white; font-size: 0.7rem; font-weight: 700; padding: 0.25rem 0.65rem; border-radius: 20px; display: flex; align-items: center; gap: 0.3rem; z-index: 1; }
    .reco-empty { text-align: center; padding: 2rem; color: #94a3b8; display: flex; flex-direction: column; align-items: center; gap: 0.75rem; }
    .skeleton-card { pointer-events: none; }
    .skeleton-img { height: 200px; background: linear-gradient(90deg, #e2e8f0 25%, #f1f5f9 50%, #e2e8f0 75%); background-size: 200% 100%; animation: shimmer 1.5s infinite; }
    .skeleton-line { background: linear-gradient(90deg, #e2e8f0 25%, #f1f5f9 50%, #e2e8f0 75%); background-size: 200% 100%; animation: shimmer 1.5s infinite; border-radius: 4px; }
...
export class FrontHomePage implements OnInit {
  ...
  recoCampsites = signal<any[]>([]);
  recoEvents = signal<any[]>([]);
  recoLoading = signal<boolean>(false);
  ...
  loadRecommendations() {
    ...
    this.recoService.getForUser(userId, 4).subscribe({
      next: (res: any) => {
        this.recoCampsites.set(res?.data?.campsites || []);
        this.recoEvents.set(res?.data?.events || []);
  ...
  openBookingModal(camp: any) {
    ...
    const normalized = {
      ...camp,
      nightlyPrice: camp.nightlyPrice ?? camp.nightly_price,
      imageUrl: camp.imageUrl ?? camp.image_url,
    };
    this.activeCamp.set(normalized);
